    ...
    afterEvaluate {
        if (project.plugins.hasPlugin("com.github.johnrengelman.shadow") &&
            ...
            val dockerTask: DockerBuildImage = tasks.create("dockerize", DockerBuildImage::class) {
                ...
                // Detect branch and set tag accordingly
                val branch = System.getenv("CI_COMMIT_REF_NAME") ?: try {
                    "git rev-parse --abbrev-ref HEAD".runCommand(dockerContextDir).trim()
                } catch (e: Exception) {
                    ""
                }
                val customTag = when (branch) {
                    "main" -> {
                        val rootVersionFile = file("${rootProject.projectDir}/VERSION")
                        if (rootVersionFile.exists()) rootVersionFile.readText().trim() else "latest"
                    }
                    "development" -> "latest"
                    else -> branch.replace(Regex("[^a-zA-Z0-9_.-]"), "-")
                }
                images.add("${project.name}:$customTag")
...
// Helper function to run shell commands from Gradle
